...
Source: current React portfolio codebase. This document captures the visible website copy, page content, case-study copy, blog summaries, navigation labels, and footer text.
...
I'm Sricharan. I build responsive, user-friendly web interfaces that are clean, fast, and functional. Currently working at Desisle, turning designs into real web experiences.
...
1 year of building real products - writing clean code, shipping fast, and growing with every project.
...
### Personal / Visionary Section

There is more to the work than the work

#### 01. Photography

- Discipline: Hobby
- Description: I photograph the way I write code - searching for the exact composition before pressing anything. Light, geometry, and the half-second before things change.
- Note: Still frames. Still thinking.

#### 02. Script writer

- Discipline: Hobby
- Description: The best architectural decisions I have made happened nowhere near a keyboard. Removing yourself from the problem is often the most productive thing you can do with a hard question.
- Note: Words before wireframes.

#### 03. Video editor

- Discipline: Hobby
- Description: A script and a codebase solve the same problem: how do you move someone step by step without losing them. Editing taught me pacing, and pacing makes everything better.
- Note: Cut to the point.

#### 04. Developer

- Discipline: Craft
- Description: Client projects sharpen execution. Personal projects sharpen taste - experiments in tooling, interface patterns, and product ideas that do not fit anywhere else.

## Case Study Detail Page Shared Labels
...
- Role: Frontend Developer
- Duration: 6 weeks
- Client Type: Diversified Conglomerate
- Source Note: Client confidential - name withheld by request.
...
- HTML & CSS - Custom frontend (build on WordPress)
- ACF setup - Custom field (configuration in WP)
- PHP bug fixing - Backend issue (resolution)
- Documentation - Full site (and editing flow docs)
...
Tags: Custom WordPress built, ACF fields configured, PHP bugs resolved, Full site documented, Client-ready handoff, HTML & CSS frontend
...
- Live - Shipped on schedule (Currently live.)
- Multi - Brand platform (Built for multiple verticals under one group.)
- Token-first - Design system (Shared shell with brand-specific overrides.)
- 6 weeks - Delivery window (Planned and shipped within the project schedule.)

#### Ecosystem Blocks

- Multi-brand identity without fragmentation: Each vertical needed its own presence within a single, coherent structure.
- Visitor routing without friction: Someone arriving for the manufacturing division should not need to decode the full corporate hierarchy to find it.
- Scalability: New product lines or subsidiaries needed to be addable without requiring structural changes to the platform.

#### Stack Signals

- Framework: Next.js App Router.
- Styling: Tailwind CSS with brand-specific token overrides.
- Animation: Framer Motion for entry transitions only, with no decorative animation.
- Deployment: Vercel.

#### Challenge Blocks

- Shared layout shell: Designing a shell that all brand sections inherit from while allowing each section to override visual properties without breaking the parent structure. Solved by separating layout tokens from brand tokens at the CSS variable level.
- Page weight management: The visual ambition of the design required discipline with asset loading strategy - lazy loading below-fold media and static generation for all content pages.

#### Result Metrics

- Live - Launched on schedule (Client confirmed and currently live.)
- Hours - New vertical setup (New verticals can be added in hours, not weeks.)
- Unified - Group identity (One corporate platform with differentiated verticals.)
...
- Reading experience at scale: Long-form financial articles needed typographic and spatial treatment that reduces fatigue across extended reading sessions.
- Authority through restraint: The visual language had to signal credibility to a financially literate audience without becoming cold or exclusionary to a general one.
- Organic growth infrastructure: Metadata, structured data, canonical logic, and page generation strategy had to be correct before the first article was published.

#### Stack Signals

- Framework: Next.js App Router with static generation for article pages.
- Content model: MDX with frontmatter for metadata management.
- SEO: JSON-LD structured data, OpenGraph image generation, and sitemap generation.
- Deployment: Vercel.

#### Challenge Blocks

- Line length and leading: Long-form content requires different typographic constraints than a marketing page. The reading column is capped at 68 characters per line at all breakpoints.
- Static generation at content volume: As the article archive grows, build time becomes a constraint. ISR with a 24-hour revalidation window balances freshness against build performance from the start.

#### Result Metrics

- Green - Core Web Vitals (Article pages achieved green scores across all metrics on launch day.)
- Live - Launched on schedule (Currently live.)
- SEO-first - Growth foundation (Search infrastructure was built before publication scale.)
...
- Client Type: SaaS Design Agency
- Live URL: https://www.desisle.com/
- Source Note: Result metrics should be verified before publishing.
...
- Development - React / Next.js site work (beyond design)
- Testing - QA across pages (and features)
- Blog ops - Publishing (and maintaining blog posts)
- Case studies - Posting (and maintaining client work)
...
Tags: Case studies live & maintained, Blog pipeline operational, Site tested across all flows, React / Next.js codebase, Zero broken content shipped
...
- Service packaging: A studio that builds websites and SaaS products needs to present those as distinct offers with distinct value propositions, not as one undifferentiated list of capabilities.
- Proof without overwhelm: Case studies need to be visible enough to build confidence but contained enough not to become an obstacle between the visitor and the contact page.
- Contact path: The distance between intent and action should be one step. Calendly integration, not a form that enters a queue.

#### Stack Signals

- Framework: Next.js.
- Styling: Tailwind CSS.
- Animation: Framer Motion scroll-triggered reveals, with no looping animations.
- Booking: Calendly embedded inline, not redirect.

#### Challenge Blocks

- Editorial ambition vs conversion logic: Design wanted a rich, layered visual experience. Conversion logic wanted the fewest possible steps. The richness lives in the visual treatment of components, not in the quantity of components.
- Case study preview system: The homepage needed enough information to qualify interest without recreating the full case study. The card shows the outcome statement, client type, and a single result metric.

#### Result Metrics

- 4 weeks - Launch window (Launched in 4 weeks.)
- Primary - Sales asset (Currently the primary sales asset for the studio.)
- Inline - Booking path (Shortened the distance between intent and action.)
...
- Role: Frontend Developer
- Duration: 5 weeks
- Client Type: SaaS - B2B India
- Live URL: https://hosaksham.in/
- Source Note: Result metrics should be verified before publishing.
...
- Rs12Cr+ - Collected annually (via the platform)
- 100+ - SMBs trust (the product)
- 60+ - Hours saved (per business per month)
...
- Feature surface communication: Enumerating features converts nobody. Identifying the visitor's problem and showing the feature that solves it converts people who have that problem.
- Audience segmentation without separate pages: Three distinct operator types on a single URL, each experiencing a tailored version of the product story.
- Trust for a complex product: Multi-feature SaaS tools require more proof than single-purpose tools. The website needed specific, contextual proof points rather than generic testimonials.

#### Stack Signals

- Framework: Next.js.
- Styling: Tailwind CSS.
- Interactivity: React state-driven operator context switcher.
- Animation: Framer Motion context transition animations.

#### Challenge Blocks

- Operator context switcher: The switcher had to read as navigation rather than a content toggle. Visitors should feel like they are moving to a relevant section of the product, not activating a filter.
- Content parity across contexts: If the gym context is more detailed than the online programme context, visitors notice the asymmetry and draw conclusions about product maturity. All three contexts required equal depth.

#### Result Metrics

- Live - Launched on schedule (Currently live.)
- 3 - Operator contexts (Studio & Gym, Online Programme, Subscription Community.)
- Scoped - State architecture (Context state is local to the product demonstration section.)
...
- Client Type: EdTech - Cohort Program
...
- Full website build - Front to back (in React / Next.js)
- Feature sections - Curriculum, pricing (mentor, FAQ)
- Blog publishing - Content ops (and live posts)
- Testing & QA - End-to-end (across all flows)
...
Tags: Full site built & live, Blog pipeline running, All flows QA tested, Cohort 1 enrolling now, Dual product ownership, React / Next.js
...
- Product - Website type (Structured for product explanation and conversion.)
- Responsive - Frontend output (Built for desktop, tablet, and mobile flows.)
- Reusable - Components (Sections planned for easy future updates.)

#### Ecosystem Blocks

- Product clarity: The site needed to make the product easy to understand without relying on long explanations.
- Conversion path: Each section supports the next decision, from awareness to action.

#### Stack Signals

- Frontend: Responsive page development with reusable components.
- Styling: Clean section layouts, consistent spacing, and scalable UI patterns.

#### Result Metrics

- Clear - Product story (The product value is easier to scan and understand.)
- Modular - Page system (Future sections can be added without layout rework.)
...
- Client Type: SaaS Tool - Designer Utility
- Live URL: https://curiousdesigner.in/
...
- Deployment - Taking the product (live end to end)
- Feature fixes - Bug resolution (in React / Next.js)
- Testing & QA - End-to-end (functionality checks)
- Product - Resource manager (for designers)
...
Tags: Product deployed live, Feature bugs resolved, End-to-end QA complete, React / Next.js, Auth flows verified, Production-stable
...
- Utility - Product type (Designed for repeated daily use.)
- Clean - Interface (Low-friction layout for saving and scanning links.)
- Responsive - Experience (Works across device sizes.)

#### Ecosystem Blocks

- Information organisation: Saved links need enough structure to be useful without becoming hard to manage.
- Daily-use ergonomics: The interface needed to stay light, fast, and predictable for repeated use.

#### Stack Signals

- Frontend: React-style component thinking for cards, lists, and states.
- UX: Focused flows for saving, finding, and revisiting links.

#### Result Metrics

- Simple - Core flow (Save, organise, and revisit links quickly.)
- Scalable - UI structure (Categories and cards can grow with the user's collection.)
...
- Client Type: 9 companies, 34+ brands
- Source Note: Client confidential - name withheld by request.
...
- Website docs - Full flow (documented per site)
- Brand guidelines - Across the group's (websites)
- Cross-site QA - End-to-end (testing all sites)
- Enterprise scale - 9 companies (34+ brands)
...
- 9 - Companies (documented)
- 34+ - Brands covered (in guidelines)
- 2 - Languages tested (EN & AR)
- 67+ - Years of legacy (represented)

Tags: Multi-site documentation, Brand guidelines complete, End-to-end QA done, Bilingual testing, Enterprise-scale delivery, Internal team handoff
...
- Group - Website type (Built for an automotive business with multiple verticals.)
- Premium - Visual direction (Focused on vehicle imagery, hierarchy, and trust.)
- Responsive - Frontend output (Structured for mobile and desktop browsing.)

#### Ecosystem Blocks

- Brand and service clarity: Visitors should quickly understand the group, its vehicle categories, and its service offering.
- Premium presentation: The interface needed to feel polished without making enquiry paths harder to reach.

#### Stack Signals

- Frontend: Responsive website development with reusable sections.
- UI: Premium imagery, structured content blocks, and clear conversion paths.

#### Result Metrics

- Clear - Group positioning (The website presents the business and its verticals clearly.)
- Ready - Enquiry flow (Visitors can move from browsing to contact with less friction.)
...
Writing is the process of finding out whether you understand something or whether you merely believe you do. Working notes from someone actively building.
...
- sri charan.dev
- Frontend Developer - React & Next.js - Product-minded engineer
- Currently heads-down at Desisle LLP. Open to conversations about the right next thing.
- Footer links: Instagram, LinkedIn, Email
- Footer nav: About, Work, Blog, Contact
- (c) 2026 Sri Charan. Crafted with intent.
...
Outside of code, I explore the world through 24 frames per second. Video editing is my rhythmic meditation.
